...
----------------------------------------------------------------------------------
----------------------------------------------------------------------------------
set ECHO OFF TERMOUT ON TAB OFF TRIMOUT ON TRIMSPOOL ON PAGESIZE 50000 LINESIZE 500 FEEDBACK OFF VERIFY OFF COLSEP '|'
...
prompt
select distinct NAME as PARAMETER
  , VALUE 
from GV$PARAMETER 
...
select c.CON_ID
  ...
  , c.OPEN_MODE
  , c.RESTRICTED
  , case when c.OPEN_MODE not like 'READ%' and c.CON_ID = sys_context('USERENV', 'CON_ID') and c.CON_ID != 0 then 'NOT OPEN! DBA_FEATURE_USAGE_STATISTICS is not accessible. *CURRENT CONTAINER'
...
group by rollup(CON_ID), PRODUCT having not (max(CON_ID) in (-1, 0) and grouping_id(CON_ID) = 1)      -- aggregation not needed for non-container databases
