...
SELECT NAME, DB_UNIQUE_NAME, DATABASE_ROLE, OPEN_MODE, PROTECTION_MODE, PROTECTION_LEVEL, force_logging, flashback_on, log_mode, SWITCHOVER_STATUS FROM V$DATABASE;
prompt
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('RESULT: ERROR: ERRORS: ' || to_char(v_num_errors) || ' | WARNNINGS: ' || to_char(v_num_warnings));
  ELSIF v_num_warnings > 0 THEN
    DBMS_OUTPUT.PUT_LINE('RESULT: _WARN: ERRORS: ' || to_char(v_num_errors) || ' | WARNNINGS: ' || to_char(v_num_warnings));
  ELSE
    DBMS_OUTPUT.PUT_LINE('RESULT: ___OK: ERRORS: ' || to_char(v_num_errors) || ' | WARNNINGS: ' || to_char(v_num_warnings));
  END IF;
