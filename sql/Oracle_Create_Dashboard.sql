...
prompt #
prompt ##############################################################
set define off trims on newp none heads off echo off feed off pagesize 0 null null verify off wrap off timing off serveroutput off termout off heading off feedback off
...
prompt ##############################################################
prompt # Variables to draw active sessions Graph, 80 samples are shown in graph.
prompt # 4 arrays are used to show session range.
prompt ##############################################################
...
pv_work_done  int := 0;
prompt ##############################################################
prompt # Fbyt function format bytes in K,M,G,T
prompt ##############################################################
...
prompt ##############################################################
prompt # Initial Procedure to extend all the arrays to ASG [Active Sessions Graph ]
prompt ##############################################################
...
prompt ##############################################################
prompt # ginsteff build data set from gv$sysstat, gv$osstat, gv$dlm_misc
prompt # and load data collection name pv_last_ginst
prompt # only every loop, pv_last_ginst will be assigned to pv_first_ginst to get the delta
prompt ##############################################################
function ginsteff return ta_ginst
is
fv_ginst  ta_ginst := ta_ginst();
fv_sql    varchar2(3000) := q'[select ty_ginst(a.instance_number,a.instance_name,b.name,c.value,sysdate) from gv$instance a , gv$statname b, gv$sysstat c
                               where a.inst_id = b.inst_id and b.inst_id = c.inst_id and b.statistic# = c.statistic# and b.name in
                               ('execute count', 'parse count (hard)', 'parse count (total)','physical read total IO requests', 'physical read total bytes','physical write total IO requests', 'physical write total bytes','redo size', 'session logical reads', 'user commits', 'cell physical IO interconnect bytes returned by smart scan','cell physical IO bytes saved by storage index','cell flash cache read hits','gc cr blocks served','gc current blocks served','gc cr blocks received','gc current blocks received','gcs messages sent','ges messages sent')
                               union all
                               select  ty_ginst(d.inst_id,e.instance_name,d.stat_name,d.value,sysdate) from gv$osstat d, gv$instance e
                               where d.inst_id = e.inst_id and d.stat_name in ('IDLE_TIME','USER_TIME','SYS_TIME','IOWAIT_TIME','BUSY_TIME','NICE_TIME','NUM_CPU_CORES')
                               union all
...
prompt ##############################################################
prompt # ginstbuilddata - once data set is ready by calling ginsteff [ above function ], ginstbuilddata
prompt # format the data by query both samples [ pv_first_ginst and pv_last_ginst ]
prompt ##############################################################
...
prompt ##############################################################
prompt # updating CPU info -- updating with every sample in case new node join/leave the cluser
prompt ##############################################################
select  sum(first.value) into pv_num_cpus from table (fv_firstsample) first where first.statname = 'NUM_CPU_CORES';
prompt ##############################################################
prompt # Initializing package variables with sample time for later use.
prompt ##############################################################
...
prompt ##############################################################
prompt # Not using pv_sample as selecting across multiple instances may take more time, so would be wise to calculate sample based on timestamps
prompt ##############################################################
...
prompt ##############################################################
prompt # gash - build dataset from gv$active_Session_history for the given sample time,
prompt # added logic to eliminate the own session info
prompt ##############################################################
...
FV_SQL  := q'[select ty_gash(gash.Inst_id,gash.session_id,gash.sql_id,gash.sql_child_number, substr(sql_Text,1,100),decode(sql_plan_hash_value,0,null,sql_plan_hash_value), round(decode(IO_CELL_OFFLOAD_ELIGIBLE_BYTES,0,0,100*(IO_CELL_OFFLOAD_ELIGIBLE_BYTES-IO_INTERCONNECT_BYTES)/decode(IO_CELL_OFFLOAD_ELIGIBLE_BYTES,0,1,IO_CELL_OFFLOAD_ELIGIBLE_BYTES))), substr(nvl2(sql_profile,'SProf',null)||nvl2(sql_plan_baseline,'SBase',null)||nvl2(sql_patch,'SPatc',null)||nvl2( decode(IS_RESOLVED_ADAPTIVE_PLAN,'Y','Y'),'ADapt',null),1,9),
...
prompt ##############################################################
prompt # gashbuilddata - it access data from collection pv_only_gash built in gash function for given sample
prompt ##############################################################
...
FOR II IN (SELECT PLANHASH, PCT,SQL_ID||'('||SQL_CHILD||')' SQL_ID, OFFLOADPCT,PLANCNTRL, pgamb, tmpmb  FROM 
          ...
          (SELECT SQL_ID,SQL_CHILD, PLANHASH, max(OFFLOADPCT) OFFLOADPCT,min(PLANCNTRL) PLANCNTRL, max(pgamb) pgamb, max(tmpmb) tmpmb, SUM(TIME_WAITED) SQLTIME FROM TABLE(FV_ONLY_GASH) where sql_id is not null group by sql_id,sql_child, planhash ) sqlt,
          (select sum(time_Waited) tottime from  table(fv_only_gash)) tot order by pct desc) where rownum < 6) loop
           --- building inst_id and sid string for top sqls
           select substr(sidstring,1,66) into fv_sid_string from 
          (select listagg(inst_id||':'||sid,', ') within group ( order by sql_id||'('||sql_child||')' ) sidstring FROM 
          (SELECT DISTINCT SQL_ID,SQL_CHILD,INST_ID,SID FROM TABLE(FV_ONLY_GASH) WHERE SQL_ID||'('||SQL_CHILD||')' = II.SQL_ID));
